...
with files stored in SharePoint by extending the
...
The [sharepointr](https://github.com/elipousson/sharepointr) package
seeks to ease this challenge by wrapping the `{Microsoft365}` methods
and adding support for the specification of SharePoint sites, drives,
...
The most useful functions for most users may be
...
[`read_sharepoint()`](https://elipousson.github.io/sharepointr/reference/read_sharepoint.md)
downloads a SharePoint file to a temporary folder and then, based on the
file extension, reads it into your environment using an appropriate
function and package (e.g. using
[`officer::read_docx()`](https://davidgohel.github.io/officer/reference/read_docx.html)
for Microsoft Word files or
...
returns a `ms_drive_item` object and supports parsing for shared file
and folder URLs:
...
that adds support for downloading copies of your SharePoint files (this
is the function that powers
[`read_sharepoint()`](https://elipousson.github.io/sharepointr/reference/read_sharepoint.md)):
...
[Microsoft365R](https://github.com/Azure/Microsoft365R) S6 object
methods already return a data frame by default such as the `list_items`
and `list_files` methods for
...
wraps this method to return a data frame similar to the output from
...
and
[`get_sp_item()`](https://elipousson.github.io/sharepointr/reference/get_sp_item.md),
that can parse a URL to determine the SharePoint site URL, drive name,
and file path based on the URL.

[`get_sp_site()`](https://elipousson.github.io/sharepointr/reference/sp_site.md)
is a minimal wrapper for
...
[`get_sp_drive()`](https://elipousson.github.io/sharepointr/reference/sp_drive.md)
returns a `ms_drive` object but required a SharePoint URL that includes
the name of the drive (otherwise it returns the drive named in the
