...
`list_sp_plan_buckets()` lists the buckets for a specified plan using
the `list_buckets` method.
...
- as_data_frame:
  ...
  If `TRUE` (default for `list_sp_plan_buckets()`), return a data frame
  of object properties with with a list column named "ms_plan_bucket"
  containing `ms_plan_task` objects. If `FALSE`, return a
...
For `list_sp_plan_buckets()`, a list of `ms_plan_bucket` class objects
or a data frame with a list column named "ms_plan_task".
